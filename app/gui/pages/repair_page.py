...
class RepairPage(BasePage):
    def create_widgets(self):
        ...
        # Encabezado
        header = ctk.CTkLabel(self.content_frame, text="Reparar PDF", font=(Theme.FONT_FAMILY, 24, "bold"), text_color=Theme.TEXT_MAIN)
        header.pack(pady=20)
        
        # Descripción
        desc_frame = ctk.CTkFrame(self.content_frame, fg_color="transparent")
        desc_frame.pack(fill="x", padx=40, pady=10)
        desc_lbl = ctk.CTkLabel(desc_frame, text="Intenta reconstruir la estructura y la tabla de referencias (XREF) de archivos dañados.\nÚtil si el PDF no se abre o muestra errores al cargar.",
        ...
        ctk.CTkLabel(input_frame, text="PDF Dañado:", font=(Theme.FONT_FAMILY, 14, "bold"), text_color=Theme.TEXT_MAIN).pack(side="left", padx=20, pady=20)
        ...
        # Botón de Acción
        ctk.CTkButton(self.content_frame, text="Reparar PDF", command=self.repair_pdf, 
    ...
    def repair_pdf(self):
        ...
        try:
           success = self.backend.repair_pdf(input_path, output_path)
           if success:
               messagebox.showinfo("Éxito", "El PDF ha sido reparado (reconstruido) exitosamente.")
               self.full_input_path = ""
               self.file_label.configure(text="Ningún archivo seleccionado", text_color="gray")
           else:
               messagebox.showerror("Error", "No se pudo reparar el archivo PDF.\nEs posible que el daño sea demasiado severo.")
