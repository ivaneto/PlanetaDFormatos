...
class Ppt2PdfPage(BasePage):
    def create_widgets(self):
        self.selected_file = None
        
        # Encabezado
        header = ctk.CTkLabel(self.content_frame, text="PowerPoint a PDF", font=(Theme.FONT_FAMILY, 24, "bold"), text_color=Theme.TEXT_MAIN)
        header.pack(pady=20)
        ...
        ctk.CTkButton(select_frame, text="Seleccionar archivo PowerPoint", command=self.select_file,
    ...
    def select_file(self):
        filename = filedialog.askopenfilename(
            title="Seleccionar archivo PowerPoint",
            filetypes=[("Archivos PowerPoint", "*.pptx *.ppt")]
        )
        if filename:
    ...
    def convert_file(self):
        ...
        if output_path:
            try:
                Converter.powerpoint_to_pdf(self.selected_file, output_path)
